def main():
    ...
    #check to see that the database extraction was correct...that there are the correct number of columns
...
######################## DEAL WITH FORMATTING ISSUES THAT SHOULD LATER BE TAKEN CARE OF DIRECTLY WITHIN THE DATABASE ITSELF #############################
    ...
    uw2 = uw.copy() #making a copy is necessary for df.loc to work as intended#
    ...
    # for gnats discrete data, set the depths all to zero, rather than half zero, half null
    if dict_args['gnats'][0]==2:
...
############### OTHER FORMATTING PRIOR TO MERGE ########################
    # Drop uw columns from discrete file that are automatically pulled out from the database extraction:    
...
#################### NOW WE ARE READY TO MERGE THE UNDERWAY AND DISCRETE FILES #############
    # manually find uw/d nearest dt matches within 5 minutes
    uw_nearest_dts = []
    d_nearest_dts = []
    d_unmatched_dts = []
    ...
    # Merge the nearest dfs by merge_asof with direction nearest, merge the unmatched dfs normally, concat the two merged dfs and sort by dt
